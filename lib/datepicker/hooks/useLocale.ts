...
async function getLocale(
  ...
  try {
    const localeModule = await (
      localePaths[localeString] || localePaths['en-US']
    )()
    return localeModule.default as unknown as Locale
  } catch (error) {
...
const useLocale = () => {
  ...
  if (!context) {
    throw new Error('useLocale must be used within a DatepickerProvider')
  }
  ...
  useEffect(() => {
    if (!locale) {
      ...
      getLocale(lang as keyof typeof localePaths).then(setLocale)
    }
  }, [locale, setLocale])
}
